...
start_date or as None and data will be taken from past 5 years'''
...
for i in range(len(holdings)):
    ...
    holdings_db.create_update_table(holdings[i,0], holdings[i,1], mov_avgs=False)

for j in range(len(prospects)):
    print(prospects[j,0])
    #prospects_db.remove_table(prospects[j,0])
    prospects_db.create_update_table(prospects[j,0], prospects[j,1], mov_avgs=True)

print(holdings_db.create_update_overview_table(holdings[:,0], holdings[:,1], holdings[:,2]))
print(prospects_db.create_update_overview_table(prospects[:,0], prospects[:,1], prospects[:,2]))
